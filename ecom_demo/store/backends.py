...
class MyBackEnd(object): 
    ...
    '''def authenticate(self,email, password):   # get the user credentils
        ...
        # existing_user = User.objects.get(email=email).first() # getting the info from db if the user is already exis or not  
        existing_user = user_info.objects.get(email=email)
